...
func (r *WarrantyRepo) GetByID(id int) (*model.WarrantyListItem, error) {
	...
	query := `
		SELECT w.*, c.name as customer_name, c.phone as customer_phone, i.type as type
		FROM warranties w
...
func (r *WarrantyRepo) SearchEligibleItems(keyword string, invType string) ([]model.WarrantySearchItem, error) {
	...
	if keyword != "" {
		query += ` AND (p.imei LIKE ? OR c.phone LIKE ? OR c.name LIKE ? OR p.model_name LIKE ? OR ii.description LIKE ?)`
		kw := "%" + keyword + "%"
